...
class CabacEncoder:
    ...
    def flush(self):
        ...
        self.L=self.L+self.R
        self.renormalization()
        self.put_bit((self.L>>9)&1)
        last_bits=self.L>>3
        last_bit=(last_bits>>1)&1
        self.output_buffer.append(last_bit)
...
if __name__ == "__main__":
    ...
    number00 = number0 / (number1 + number0)
    number11 = number1 / (number1 + number0)
    entropy = (number0 + number1) * (number00 * math.log(1 / number00, 2) + number11 * math.log(1 / number11, 2))
    print(entropy)
    efficiency=entropy/output.__len__()
    print('{:.2f}%'.format(efficiency*100))